...
const AuthorPanel = () => {
 ...
 const handleGenerateStory = () => {
    ...
    setText('');
    setDraftText('');
    const storyRef = ref(database, `stories/${newStoryId}`);
     set(storyRef, {content: ''})
                .then(() => {
                    handleOpen();
                    setLoading(false);
                })
    ...
    return (
        <div>
             <Button onClick={handleLogout} variant="gradient" className="mt-6 mx-2">
                  ...
                  <>
        <Button onClick={handleGenerateStory} variant="gradient" className="mt-6 mx-2">
                    Generate Story
                </Button>

      <Dialog open={open} handler={handleOpen}>
        <DialogHeader>
               Copy the link and sent it to the editor.
        </DialogHeader>
        <DialogBody divider>
                <input
                    type="text"
                    value={`${window.location.origin}/editor/${storyId}`}
                    readOnly
        ...
        <DialogFooter>
          ...
          <Button variant="gradient" color="green" onClick={handleNavigation}>
            <span>Confirm</span>
          </Button>
        </DialogFooter>
      </Dialog>
